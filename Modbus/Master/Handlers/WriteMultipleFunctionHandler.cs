...
/// <summary>
/// 写多个线圈/保持寄存器功能码处理器（FC15, FC16）
/// </summary>
internal sealed class WriteMultipleFunctionHandler : IModbusFunctionHandler
{
    internal WriteMultipleFunctionHandler(FunctionCode functionCode)
    {
    ...
    public ModbusResponseData ParseResponsePdu(ReadOnlySpan<byte> pduBody)
    {
        ...
        var quantity = TouchSocketBitConverter.BigEndian.To<ushort>(pduBody.Slice(2));
        return new ModbusResponseData(ReadOnlyMemory<byte>.Empty, startingAddress, quantity);
    }
}
